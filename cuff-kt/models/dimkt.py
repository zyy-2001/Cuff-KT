...
class DIMKT(Module):
    def __init__(self,convert,num_skills,num_questions,method,rank,embedding_size,dropout,batch_size,difficult_levels=100):
        super().__init__()
        ...
        if self.method == 'cuff' or self.method == 'cuff+':
            self.generator = common.Generator(self.num_skills, self.embedding_size, self.embedding_size, self.embedding_size, self.rank)
    ...
    def forward(self,feed_dict):
        ...
        if self.convert:
            ...
            state = logits
            correct_rates = feed_dict['correct_rates']
            y = (logits * one_hot(cshft.long(), self.num_skills)).sum(-1)
        else:
