...
class ATDKT(Module):
    def __init__(self, num_skills, num_questions, method, rank, embedding_size, dropout=0.1, 
        ...
        if self.method != 'cuff' and self.method != 'cuff+':
            self.out_layer = nn.Sequential(
                nn.Linear(self.hidden_size, self.hidden_size//2), nn.ReLU(), nn.Dropout(dropout),
                Linear(self.hidden_size//2, self.num_skills))
        else:
            self.generator = common.Generator(self.num_skills, self.num_skills, self.embedding_size, self.hidden_size, self.rank)
    ...
    def predcurc(self, dcur, q, c, r, xemb, train, attention_reweight):
        y2, y3 = 0, 0
        ...
        if train:
            ...
            rsm = sm[:,start:]
            rflag = rsm==1
            rtrues = dcur["historycorrs"][:, :-1][:,start:]
        ...
        if self.method != 'cuff' and self.method != 'cuff+':
            ...
            y = self.generator(h, r, c, attention_reweight)
        y = torch.sigmoid(y)
        return y, y2, y3

    def forward(self, feed_dict): ## F * xemb
        # print(f"keys: {dcur.keys()}")
        q, c, r = feed_dict["questions"][:, :-1], feed_dict["skills"][:, :-1], feed_dict["responses"]
        cshft = feed_dict['skills'][:, 1:]
        ...
        y, y2, y3 = self.predcurc(feed_dict, q, c, r_input, xemb, self.training, attention_reweight)
        state = y

        out_dict = {
            "pred": (y * one_hot(cshft, self.num_skills)).sum(-1),
            "true": r[:, 1:].float(),
            "y": y,
            "y2": y2,
            "y3": y3,
